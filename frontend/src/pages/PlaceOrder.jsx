...
const PlaceOrder = () => {
    ...
    // --- AUTO-FETCH ADDRESS BY PINCODE ---
    const handlePincodeChange = async (e) => {
        const pin = e.target.value;
        setFormData(prev => ({ ...prev, zipcode: pin }));

        // Rigorous check for Indian Pincodes (6 digits)
        if (pin.length === 6) {
            setIsVerifying(true);
            try {
                ...
                if (response.data[0].Status === "Success") {
                    ...
                    toast.success(`Registry verified: ${data.District}, ${data.State}`);
                } else {
                    toast.error("Invalid Pincode. Location not found in Archive.");
                }
            } catch (error) {
    ...
    const validateFinalRegistry = () => {
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!emailRegex.test(formData.email)) return "Invalid Registry Email.";
        if (formData.phone.length < 10) return "Contact number must be at least 10 digits.";
        if (!formData.city || !formData.state) return "Location must be verified via Pincode.";
        return null;
    }

    const onSubmitHandler = async (event) => {
        event.preventDefault()
        const error = validateFinalRegistry();
        ...
        try {
            ...
            if (response.data.success) {
                setCartItems({})
                navigate('/orders')
                toast.success("Consignment Registered Successfully")
            } else {
    ...
    return (
        <form onSubmit={onSubmitHandler} className='bg-[#0a0a0a] min-h-screen text-white flex flex-col lg:flex-row justify-between gap-12 pt-10 pb-20 px-6 md:px-16 lg:px-24 select-none animate-fade-in'>
            ...
            <div className='flex flex-col gap-6 w-full lg:max-w-[550px]'>
                ...
                <div className='flex gap-4'>
                    <input readOnly name='city' value={formData.city} className='bg-[#0d0d0d] border border-white/5 rounded-sm py-4 px-5 w-full text-sm text-gray-500 outline-none' type="text" placeholder='City (Auto-filled)' />
                    <input readOnly name='state' value={formData.state} className='bg-[#0d0d0d] border border-white/5 rounded-sm py-4 px-5 w-full text-sm text-gray-500 outline-none' type="text" placeholder='State (Auto-filled)' />
                </div>
