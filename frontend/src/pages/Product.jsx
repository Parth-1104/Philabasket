...
const Product = () => {
  ...
  const getWatermarkedUrl = (url) => {
    ...
    const watermarkTransform =
      "l_Logo-5_asqxkr,fl_relative,w_0.7,c_scale,o_80,a_-45";
  ...
  const discount =
    productData && productData.marketPrice > productData.price
  ...
  if (!productData)
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white">
        ...
        <p className="text-[10px] font-black tracking-[0.6em] text-gray-300 uppercase">
          ...
          <div className="bg-white/90 backdrop-blur-xl border border-black/5 shadow-2xl rounded-2xl p-4 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-[9px] font-black tracking-[0.2em] text-gray-700 uppercase">
                  Specimen Secured
                </p>
              </div>
              ...
              <p className="text-[9px] font-bold text-[#BC002D] mt-0.5">
                Qty: {quantity} • Synced
              </p>
            </div>
            <button
              onClick={() => setShowPopup(false)}
              className="p-1.5 hover:bg-gray-100 rounded-full shrink-0 transition-colors text-gray-700"
            >
              <X size={14} />
            </button>
          </div>
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-4 md:px-12 lg:px-16">
        ...
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-16 items-start">
          ...
          <div className="w-full lg:w-[38%] md:w-[70%] lg:sticky lg:top-18 flex flex-col gap-4">
            {/* MAIN VIEWER */}
            <div
              className="relative w-full bg-[#F7F7F7] rounded-3xl overflow-hidden group shadow-sm"
              style={{
            ...
            >
                ...
                {isVideoActive && activeVideo ? (
                  <div className="absolute inset-0 bg-black z-20 animate-fade-in">
                    <button
                      ...
                      className="absolute top-4 right-4 z-30 bg-white/20 backdrop-blur-md text-white p-2.5 rounded-full hover:bg-[#BC002D] transition-colors"
                    >
                      <X size={18} />
                    </button>
                    <iframe
                      className="w-full h-full"
                      src={getEmbedUrl(productData.youtubeUrl)}
                      frameBorder="0"
                  ...
                  <div
                    ...
                    className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-gray-900 cursor-pointer group/vid"
                  >
              ...
              {totalMedia > 1 && (
                <>
                  <button
                    onClick={prev}
                    className="absolute left-3 top-1/2 -translate-y-1/2 z-10 w-9 h-9 bg-white/80 backdrop-blur-sm rounded-full flex items-center justify-center shadow-lg hover:bg-white active:scale-95"
                  >
                    <ChevronLeft size={16} className="text-gray-800" />
                  </button>
                  <button
                    onClick={next}
                    className="absolute right-3 top-1/2 -translate-y-1/2 z-10 w-9 h-9 bg-white/80 backdrop-blur-sm rounded-full flex items-center justify-center shadow-lg hover:bg-white active:scale-95"
                  >
            ...
            <div className="flex items-center justify-center py-1.5 border-y border-gray-50 bg-gray-50/50 rounded-lg">
              <p className="text-[10px] font-bold text-gray-700 uppercase tracking-widest flex items-center gap-1.5">
                ...
                <span className="text-[#BC002D] font-black">
                  {potentialPoints} Rewards Points
                </span>
              </p>
            </div>
              ...
              <div className="flex flex-col items-start">
                <div className="flex items-baseline gap-3">
                  ...
                  <span className="text-3xl md:text-4xl font-bold text-gray-900 tracking-tighter">
                    {valuationSymbol}
                    {String(formatPrice(productData.price))
                      .replace(/[₹$]/g, "")
                      .trim()}
                  </span>
                  ...
                  {productData.marketPrice > productData.price && (
                    <span className="text-sm font-bold text-gray-300 line-through">
                      {valuationSymbol}
                      {String(formatPrice(productData.marketPrice))
                        .replace(/[₹$]/g, "")
                        .trim()}
                    </span>
                  )}
              ...
              <div className="flex flex-col gap-2 p-4 bg-[#BC002D]/5 rounded-2xl border border-[#BC002D]/10 hover:bg-[#BC002D]/8 transition-all">
                <div className="flex items-center gap-2">
                  <ShieldCheck
                    ...
                    strokeWidth={2.5}
                  />
                  <span className="text-[9px] font-black text-[#BC002D] uppercase tracking-widest">
                    Condition
                  </span>
                </div>
              ...
              <div className="flex flex-col gap-2 p-4 bg-[#BC002D]/5 rounded-2xl border border-[#BC002D]/10 hover:bg-[#BC002D]/8 transition-all">
                ...
                <div className="flex flex-wrap gap-1 ml-[22px]">
                  {productData.category &&
                    productData.category.slice(0, 2).map((cat, i) => (
                      <span
                        key={i}
                        className="text-[10px] font-black text-gray-900 capitalize truncate"
                      >
                        {cat}
                        {i < productData.category.slice(0, 2).length - 1
                          ? ","
                          : ""}
                      </span>
                    ))}
