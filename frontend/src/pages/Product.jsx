...
const Product = () => {
  ...
  // --- THE FIX: INTELLIGENT CACHE CHECK ---
  useEffect(() => {
    if (!image) return;
    ...
    // If the browser already has the image, skip the loading animation
    if (img.complete) {
          ...
          <div className='flex lg:flex-col overflow-x-auto lg:w-[12%] w-full hide-scrollbar gap-3'>
              {productData.image.map((item, index) => (
                  <img 
                    ...
                    src={item} 
                    key={index} 
                    className={`w-[18%] lg:w-full aspect-square object-contain cursor-pointer border transition-all duration-500 p-1.5 bg-[#F9F9F9] ${image === item ? 'border-[#BC002D]' : 'border-black/5 opacity-50 hover:opacity-100'}`} 
                    alt="Thumbnail" 
                  />
              ))}
          </div>

          <div className='w-full lg:w-[88%] relative group bg-[#F9F9F9] flex items-center justify-center border border-black/5 p-8 min-h-[350px] md:min-h-[450px] overflow-hidden'>
              
              {/* LOADER - Now only shows if NOT in cache */}
              {!isMainLoaded && (
        ...
        <div className='flex-1 lg:max-w-[480px]'>
          ...
          <div className='mb-10 flex flex-col sm:flex-row gap-3'>
            {productData.stock > 0 ? (
              ...
              <button disabled className='w-full border border-black/5 text-gray-300 py-4 text-[10px] font-black uppercase'>Exhausted</button>
            )}
          </div>
        </div>
      </div>
