...
const ProductItem = ({ id, image, name, price, marketPrice, category, linkToFilter = false }) => {
  ...
  return (
      ...
      >
          {/* Main Container */}
          <div className='relative overflow-hidden bg-white p-1.5 lg:p-1.5 transition-all duration-700 border border-black/[0.05] group-hover:border-[#BC002D]/30 rounded-br-[30px] lg:rounded-br-[60px] shadow-sm group-hover:shadow-2xl'>
              ...
              {/* Inner Specimen Frame - FIXED HEIGHT & PADDING */}
              <div className='relative aspect-[4/5] flex items-center justify-center overflow-hidden bg-[#F8F8F8] group-hover:bg-white transition-colors duration-700 rounded-br-[25px] lg:rounded-br-[30px]'>
                  ...
                  {/* CHANGE: Reduced padding from p-8 to p-3 on mobile.
                      Changed w-fit/h-fit to w-full/h-full with object-contain to maximize space.
                  */}
                  <img 
                      draggable="false"
                      className='z-10 w-full h-full object-contain p-2 lg:p-5 filter grayscale-[0.3] group-hover:grayscale-0 drop-shadow-[0_5px_15px_rgba(0,0,0,0.08)] transform group-hover:scale-105 transition-transform duration-[1.5s] ease-out' 
