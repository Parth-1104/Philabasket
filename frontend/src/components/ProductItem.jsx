...
const ProductItem = ({
  ...
  const potentialPoints = useMemo(() => {
    ...
    let multiplier = 0.1; // Default Silver

    if (userTier === "Gold") multiplier = 0.3;
    if (userTier === "Platinum") multiplier = 0.5;
  ...
  const optimizedImage = useMemo(() => {
    ...
    const watermarkTransform =
      "l_Logo-5_go95bd,fl_relative,w_0.7,c_scale,o_90,a_-45";
    if (rawUrl.includes("f_auto,q_auto")) {
      return rawUrl.replace(
        "/f_auto,q_auto/",
        `/f_auto,q_auto,${watermarkTransform}/`,
      );
    }
    return rawUrl.replace(
      "/upload/",
      `/upload/f_auto,q_auto,w_600,${watermarkTransform}/`,
    );
  }, [image]);
  ...
  return (
    <div
      className={`relative block group select-none transition-all duration-700 w-full bg-white ${isOutOfStock ? "opacity-80" : ""}`}
    >
      {/* IMAGE CONTAINER */}
      <div
        className="relative aspect-square overflow-hidden bg-[#FDFDFD] border border-black/[0.03] group-hover:border-[#BC002D]/40 transition-all duration-500"
      ...
      <div className="py-4 px-4">
        <div className="flex flex-col gap-2">
          {/* NAME - FULL WIDTH */}
          <p
            className="text-[10px] lg:text-[13px] font-semibold tracking-tight text-gray-900 group-hover:text-[#BC002D] transition-colors leading-[1.2] line-clamp-3 min-h-[2.4em] overflow-hidden cursor-pointer w-full"
            onClick={handleNavigation}
          >
          ...
          <div className="flex items-center gap-3">
            <p className="text-[12px] lg:text-[18px] font-black text-gray-900 tabular-nums leading-none">
              <span className="text-[10px] lg:text-[12px] mr-0.5 text-[#BC002D]">
                {symbol}
              </span>
              {formatPrice(price)}
            </p>
          ...
          <div className="mt-2 flex items-center gap-2">
            ...
            {isOutOfStock ? (
              ...
              <button
                ...
                className="group relative flex-1 h-[50px] flex items-center justify-center bg-[#000000] text-white rounded-xl overflow-hidden hover:bg-[#BC002D] transition-all duration-500 ease-in-out shadow-lg shadow-[#BC002D]/10"
              >
                ...
                <div className="flex items-center gap-2 group-hover:translate-x-12 group-hover:opacity-0 transition-all duration-500 ease-in-out">
                  <ShoppingCart size={16} className="opacity-70" />
                  <span className="text-[5px] lg:text[7px] font-black uppercase tracking-[0.25em]">
                    Add to Cart
                  </span>
                </div>
