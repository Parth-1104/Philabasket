    ...
    <div className="z-[10000] w-full bg-white">
      ...
      <div className="grid grid-cols-3 items-center py-4 px-[6%] bg-white border-b border-black/[0.03] lg:p-[14px] w-full shadow-sm">
        ...
        <nav className="hidden xl:flex justify-center items-center gap-6">
          ...
          <NavLink to="/updates" className="group">
            <p className='text-[13px] font-[800] text-black group-hover:text-[#BC002D] font-["Nunito",sans-serif] mr-7'>
              Updates
            </p>
          </NavLink>
          <NavLink to="/gifting" className="group">
            <p className='text-[13px] font-[800] text-black group-hover:text-[#BC002D] font-["Nunito",sans-serif]'>
              Gifting
            </p>
          </NavLink>
        </nav>
        ...
        <div className="flex justify-end items-center gap-4 lg:gap-6">
          ...
          <Link
            ...
            className="relative hidden md:block group flex flex-col items-center"
          >
            <Heart
              size={18}
              color="#cc3314"
            ...
            />
            {wishlist.length > 0 && (
              <span className="absolute -top-2 -right-2 bg-black text-white text-[7px] w-4 h-4 rounded-full flex items-center justify-center font-black">
                {wishlist.length}
              </span>
            )}
            <div className="absolute top-8 scale-0 transition-all rounded bg-black px-2 py-1 text-[7px] font-black text-white group-hover:scale-100 uppercase tracking-widest z-[100]">
            ...
            {token && (
              <div className="group-hover:block hidden absolute right-0 pt-5 w-48 z-[110]">
                <div className="bg-white border-t-2 border-[#BC002D] p-5 shadow-2xl rounded-br-[30px]">
                  ...
                  <p
                    onClick={logout}
                    className="text-[9px] text-[#BC002D] cursor-pointer font-black uppercase flex items-center gap-2"
                  >
          ...
          >
            ...
            <p className="absolute -right-1.5 -top-1.5 w-4 h-4 text-[8px] flex items-center justify-center bg-[#BC002D] text-white rounded-full font-black">
              {getCartCount()}
            </p>
            <div className="absolute top-10 scale-0 transition-all rounded bg-black px-2 py-1 text-[7px] font-black text-white group-hover:scale-100 uppercase tracking-widest z-[100]">
              Cart
            </div>
          </div>
          ...
          <Menu
            ...
            size={22}
            className="cursor-pointer xl:hidden text-gray-900"
          />
        </div>
      </div>
      ...
      {/* --- MOBILE DRAWER --- */}
      <div
        className={`fixed inset-0 z-[5000] transition-all duration-500 ${visible ? "visible" : "invisible"}`}
      >
        <div
          onClick={() => setVisible(false)}
          className={`absolute inset-0 bg-black/80 backdrop-blur-sm transition-opacity duration-500 ${visible ? "opacity-100" : "opacity-0"}`}
        ></div>
        <div
          className={`absolute top-0 right-0 h-full w-[85%] bg-white transition-transform duration-500 ease-out ${visible ? "translate-x-0" : "translate-x-full"}`}
        >
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8">
              ...
              <div
                ...
                className="bg-black p-5 rounded-2xl flex items-center justify-between"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[8px] text-white/50 font-bold uppercase">
                      +50 Points
                    </p>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <p className="text-[8px] font-black text-[#BC002D] uppercase tracking-[0.6em] mb-4">
                  Categories
                </p>
                ...
                {headerData.navMenu.map((menu, idx) => (
                  <div key={idx} className="border-b border-gray-100 pb-2">
                    ...
                    >
                      <p className="text-xl font-bold tracking-tighter text-gray-900 capitalize">
                        {menu.title}
                      </p>
                      <ChevronRight
                        size={18}
                        className={`transition-transform duration-300 ${expandedMenu === idx ? "rotate-90 text-[#BC002D]" : "text-[#BC002D]"}`}
                      />
                    </div>
                            ...
                            <div className="flex flex-col gap-3">
                              {group.items.map((item, iIdx) => (
                                <Link
                                  ...
                                  to={`/collection?category=${encodeURIComponent(item)}`}
                                  onClick={() => setVisible(false)}
                                  className="text-[13px] font-bold text-gray-600 hover:text-[#BC002D] transition-colors"
                                >
                                  {item}
                                </Link>
                              ))}
            ...
            <div className="p-8">
              ...
              >
                ...
                <RefreshCw
                  size={10}
                  className="text-gray-300 group-hover:text-[#BC002D] ml-auto transition-colors"
                />
              </button>
              {token ? (
                <button
                  ...
                  className="w-full py-5 bg-gray-900 text-white text-[11px] font-black uppercase tracking-[0.4em] rounded-xl flex items-center justify-center gap-2"
                >
                ...
                <button
                  ...
                  className="w-full py-5 bg-[#BC002D] text-white text-[11px] font-black uppercase tracking-[0.4em] rounded-xl"
                >
                  Login
                </button>
              )}
