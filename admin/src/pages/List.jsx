    ...
    <div className='max-w-7xl mx-auto p-4 lg:p-6 bg-gray-50 min-h-screen'>
      ...
      <div className='flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4'>
        <div>
          ...
          <p className='text-sm text-gray-500'>Live stock management for archive stamps</p>
        </div>
        ...
        {selectedItems.length > 0 && (
          <div className='flex items-center gap-3 bg-white p-2 px-4 rounded-full shadow-sm border border-red-100 animate-pulse'>
            ...
            <button onClick={deleteSelected} className='bg-red-600 text-white text-[10px] px-3 py-1 rounded-full uppercase font-black hover:bg-red-700 transition-colors'>
              Delete Bulk
            </button>
          </div>
        )}
              ...
              {editingId === item._id ? (
                ...
                <>
                  <div className='text-center md:text-left'>
                    <h3 className={`font-bold line-clamp-1 ${isOutOfStock ? 'text-gray-400' : 'text-gray-900'}`}>{item.name}</h3>
                  ...
                  <div className='flex flex-wrap gap-1 justify-center md:justify-start'>
                    {item.category.slice(0, 3).map((cat, i) => (
                      <span key={i} className={`px-2 py-0.5 rounded-full text-[8px] font-bold border uppercase ${isOutOfStock ? 'bg-gray-50 text-gray-300 border-gray-200' : 'bg-blue-50 text-blue-600 border-blue-100'}`}>{cat}</span>
                    ))}
                  ...
                  <div className='text-center md:text-left'>
                    <span className={`text-lg font-black ${isOutOfStock ? 'text-gray-300' : 'text-gray-900'}`}>{currency}{item.price}</span>
                  </div>
                </>
              )}
