...
const Orders = ({ token }) => {
  ...
  useEffect(() => { if (token) fetchAllOrders(); }, [token, fetchAllOrders]);

  // --- FEATURE: PRIORITY SORTING ---
  // Order: Placed -> Packing -> Shipped -> Delivered -> Others
  const sortedOrders = useMemo(() => {
    const priority = { "Order Placed": 1, "Packing": 2, "Shipped": 3, "Delivered": 4, "Cancelled": 5 };
    return [...orders].sort((a, b) => (priority[a.status] || 99) - (priority[b.status] || 99));
  }, [orders]);

  // --- FEATURE: TARGETED CSV EXPORT (Only 'Placed' Status) ---
  const exportPlacedCSV = (timeframe = 'all') => {
    ...
    const placedOrders = orders.filter(order => {
      ...
      if (timeframe === 'month') return isPlaced && orderDate.getMonth() === now.getMonth();
      return isPlaced;
    });
    ...
    <div className='p-6 bg-gray-50 min-h-screen'>
      ...
      <div className='max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4 mb-8 bg-white p-6 rounded-2xl shadow-sm border border-gray-100'>
        <div>
          ...
          <p className='text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1'>Only "Placed" orders are exported</p>
        </div>
        <div className='flex gap-2'>
            <button onClick={() => exportPlacedCSV('today')} className='px-4 py-2 bg-green-600 text-white rounded-xl text-[10px] font-black uppercase hover:bg-green-700 transition-all shadow-lg shadow-green-100'>Today's Placed</button>
      ...
      <div className='max-w-7xl mx-auto space-y-4'>
        {sortedOrders.map((order) => (
          <div key={order._id} className={`grid grid-cols-1 md:grid-cols-[0.5fr_2.5fr_1fr_1fr_1.5fr] gap-6 items-center p-6 bg-white border rounded-2xl border-gray-100 hover:border-blue-200 transition-all`}>
            ...
            <div className='cursor-pointer' onClick={() => { setActiveOrder(order); setShowAddressModal(true); }}>
              <p className='font-black text-gray-900 text-sm mb-1'>{order.items.map(i => i.name).join(', ')}</p>
            ...
            <div className='text-center'>
              ...
              <p className='text-sm font-black text-gray-900'>{order.currency === 'USD' ? '$' : '₹'}{order.amount.toFixed(2)}</p>
            </div>
            ...
            <div className='flex items-center gap-3 justify-end'>
                <select 
                    ...
                    value={order.status} 
                    className='p-2 text-[10px] font-bold border border-gray-200 rounded-lg bg-gray-50 focus:outline-none uppercase'
                >
          ...
          <div className='fixed inset-0 bg-black/60 backdrop-blur-md z-[100] flex items-center justify-center p-4'>
              <div className='bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden'>
                  <div className='p-8 bg-gray-900 text-white flex justify-between items-center'>
                      ...
                      <button onClick={() => setShowAddressModal(false)} className='text-4xl font-light'>×</button>
                  </div>
                  <div className='p-10 grid grid-cols-2 gap-8'>
                      ...
                      <div className='bg-gray-50 p-6 rounded-xl relative'>
                          ...
                          <button onClick={() => copyAddress(activeOrder.address)} className='mt-4 w-full bg-black text-white py-2 rounded-lg text-[10px] font-black uppercase'>Copy Address</button>
                      </div>
                  </div>
              </div>
          </div>
      ...
      {showTrackingModal && (
        <div className='fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4'>
          <div className='bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in duration-200'>
            <div className='p-6 bg-blue-600 text-white'><h4 className='text-lg font-black italic'>India Post Entry</h4></div>
            <div className='p-6 space-y-4'>
              <input autoFocus placeholder="Consignment Number" className='w-full border-2 border-gray-100 p-3 rounded-xl font-mono text-sm uppercase'
                value={tempTracking} onChange={(e) => setTempTracking(e.target.value.toUpperCase())} />
              <div className='flex gap-2'>
                <button onClick={() => updateOrder(activeOrder._id, activeOrder.status, tempTracking)} className='flex-1 bg-black text-white py-3 rounded-xl font-black text-xs uppercase'>Save & Notify</button>
                <button onClick={() => setShowTrackingModal(false)} className='flex-1 bg-gray-100 text-gray-500 py-3 rounded-xl font-black text-xs uppercase'>Cancel</button>
              </div>
            </div>
          </div>
        </div>
