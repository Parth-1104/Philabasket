...
const syncMediaToProducts = async (req, res) => {
    ...
    try {
        ...
        for (const asset of orphanedMedia) {
            // 1. EXTRACT CODE: Captures everything from '#' until a SPACE or a DOUBLE HYPHEN '--'
            const fileMatch = asset.originalName.match(/#(.+?)\s*-/i);

            if (fileMatch && fileMatch[1]) {
                const productCode = fileMatch[1].trim();

                // 2. ESCAPED SEARCH: Escape special characters like '(' and ')'
                const escapedCode = productCode.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

                const product = await productModel.findOne({
                    // FIX: Look for exact code, followed by optional spaces (\s*), and then STRICTLY a hyphen or end of string (?:-|$)
                    // This prevents `#MS1 ` from accidentally matching `#MS1 (T)`
                    name: { $regex: `^#${escapedCode}\\s*(?:-|$)`, $options: 'i' }
                });
